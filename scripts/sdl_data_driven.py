...
#             name_dynamics = "motifDynamics_new/SAMPLES-10000_NTWK-"+ntwk+"_K-"+str(num_nodes)+'_DYNAMIC-'+str(ca)+'_PARAMS-csv.pkl'
#             df_dynamics = pickle.loads(s3_bucket.Object(name_dynamics).get()['Body'].read())
#             name_coladj = "motifDynamics_new/SAMPLES-10000_NTWK-"+ntwk+"_K-"+str(num_nodes)+'_COLADJ-'+str(ca)+'_PARAMS-csv.pkl'
#             df_coladj = pickle.loads(s3_bucket.Object(name_coladj).get()['Body'].read())
#             print(name_coladj+"\n")
            
#             ##########################theory driven: NMF+logistic regression##########################
#             if ntwk == "nws-20000-1000-05":
...
#                 r = 1 #change the number of dictionary elements
...
#                 W_con, H_con = ALS(X=X_comb.loc[ind_con,].T.values, 
#                                          n_components=r, n_iter=100, a0 = 0, a1 = 0, a12 = 0, 
#                                          H_nonnegativity=True, W_nonnegativity=True, 
#                                          compute_recons_error=True, subsample_ratio=1)
...
#                 data_dict["x_con"] = X_comb.loc[ind_con,]
#                 data_dict["y_con"] = df_dynamics.y[ind_con]
                
#                 if ca != "ghm": 
#                     data_dict["x_concentrated"] = X_comb.loc[ind_con,]
#                     data_dict["y_concentrated"] = df_dynamics.y[ind_con]
                    
#                     dynamicstree_filename = "motifDynamics_new/SAMPLES-100_NTWK-tree_K-"+str(num_nodes)+'_DYNAMIC-'+str(ca)+'_PARAMS-csv.pkl'
#                     coladjtree_filename = "motifDynamics_new/SAMPLES-100_NTWK-tree_K-"+str(num_nodes)+'_COLADJ-'+str(ca)+'_PARAMS-csv.pkl'
...
#                 model = FirthLogisticRegression(wald=True) ####### change this to regular logistic regression
...
beta = 0.5
iteration = 100
r = 4 #change the number of dictionary
SDL_BCD_class_new = SDL_BCD(X=[np.concatenate([data_dict["x_dense"], data_dict["x_sparse"], data_dict["x_concentrated"]]).T,
                               np.concatenate([data_dict["y_dense"], data_dict["y_sparse"]data_dict["y_concentrated"]]).reshape(-1,1).T],  # data, label
                            X_test=[X_test.T, y_test.to_numpy().reshape(-1,1).T],
                            n_components=r, xi=xi, 
                            L1_reg = [0,0,0], L2_reg = [0,0,0], 
                            nonnegativity=[True,True,False],full_dim=False)

results_dict_new = SDL_BCD_class_new.fit(iter=iteration, subsample_size=None,
                                         beta = beta, search_radius_const = np.linalg.norm(X_train), 
                                         update_nuance_param=False, if_compute_recons_error=False, if_validate=False)
...
#             beta = 0.5
#             iteration = 100
#             r = 4 # Number of dictionaries to learn
...
#             results_dict_new = SDL_BCD_class_new.fit(iter=iteration, 
#                                                      subsample_size=None,
#                                                      beta = beta,
...
#             s3_bucket.put_object(Body= pickle.dumps(results_dict_new),
#                                  Key = f"output/sdl/{ca}/r{r}/SAMPLES-10000_NTWK-{ntwk}_K-{num_nodes}_DYNAMIC-{ca}_sdl-r{r}.pkl")
