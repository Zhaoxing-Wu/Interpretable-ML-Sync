...
def datagen_FCA_dynamics(num_nodes, kappa, pred_iter, train_iter, X):
...
def datagen_coladj(num_nodes, kappa, df_graph, df_dynamics):
    # for FCA, GHM
    ...
    for i_graph in range(df_graph.shape[1]):
        ...
        temp = []
        for i_state in range(int(df_dynamics.shape[1]/num_nodes)):
            adj = adj_mx.copy()
            
            col = np.array(df_dynamics.loc[
                i_graph,
                df_dynamics.columns[0+num_nodes *
                                    i_state]:df_dynamics.columns[num_nodes-1+num_nodes*i_state]
            ])
        ...
        df.append(np.array(temp).reshape(1, -1)[0])
...
def datagen_GHM_dynamics(num_nodes, kappa, pred_iter, train_iter, X):
...
def datagen_Kuramoto_dynamics(num_nodes, K, step, pred_iter, train_iter, skip, X):
...
def datagen_Kuramoto_coladj(num_nodes, df_graph, df_dynamics):
    # df_graph: a list of vectorized adjacency matrix (for each graph, length = k^2 if num_nodes=k)
    # df_dynamics: the dataset generated by datagen_FCA_dynamics(); all columns need to be named
    df_dynamics = df_dynamics[[c for c in df_dynamics.columns if c.startswith('s')]]
    df = pd.DataFrame()
    ...
    for i_graph in range(df_graph.shape[1]):
        ...
        for i_state in range(int(df_dynamics.shape[1]/num_nodes)):
            ...
            for i in range(num_nodes):
                ...
                for j in range(num_nodes):
                    if i < j:
                        new_col = np.subtract(max(col[i], col[j]), min(
                            col[i], col[j]), dtype=np.float32)
                        if new_col > np.pi:
...
def datagen_n2v(num_nodes, file_name, dimensions, X):
    ...
    for i in range(X.shape[1]):
        ...
        df = df.append(
            pd.Series(node2vec_model.get_embedding().reshape(1, -1)[0]), ignore_index=True)
    df.columns = header
...
def datagen_g2v(num_nodes, file_name, dimensions, X):
    ...
    for i in range(X.shape[1]):
        ...
        df = df.append(
            pd.Series(graph2vec_model.get_embedding()[0]), ignore_index=True)
    df.to_csv(file_name, index=False)
